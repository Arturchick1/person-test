...
func TestGetPersonWhenOk(t *testing.T) {
	...
	personRepository := New(storage.DB)
	ctx, _ := context.WithCancel(context.Background())
	var id int64 = 1

	person, err := personRepository.GetPerson(ctx, id)
...
func TestInsertUpdateAndDeletePerson(t *testing.T) {
	...
	id, err := personRepository.CreatePerson(ctx, personDTO)
	require.NoError(t, err)
	...
	idUpdated, err := personRepository.UpdatePerson(ctx, personUpdate, id)
	require.NoError(t, err)

	personUpdated, err := personRepository.GetPerson(ctx, idUpdated)
	require.NoError(t, err)
	...
	err = personRepository.DeletePerson(ctx, id)
	require.NoError(t, err)

	p, err := personRepository.GetPerson(ctx, id)
	require.Error(t, err)
